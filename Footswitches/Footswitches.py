...
class Looper_Switches(MCP23017_R1.MCP23017, object):
    ...
    allPins = [BIT_A0, BIT_A1, BIT_A2, BIT_A3, BIT_A4, BIT_A5, BIT_A6, BIT_A7, BIT_B0, BIT_B1, BIT_B2, BIT_B3, BIT_B4,
               BIT_B5, BIT_B6, BIT_B7]
    ...
    def __init__(self, address=0x22, busnum=I2C.get_default_bus()):
        ...
        self.read_gpio(self.allPins)
        self.ioconSetup(0x02)  # active high interrupt polarity

    def read_gpio(self, pins):
        give_me_gpio = self.input_pins(pins)
        return give_me_gpio

    def read_interrupt_cap_pin(self, pin):
        return self.interruptCapture(pin)

    def enable_interrupt_pin(self, pin):
        self.gpinten_pin(pin, True)

    def disable_interrupt_pin(self, pin):
        self.gpinten_pin(pin, False)

    def interrupt_flag_register(self, bank):
